...
#if (defined(NEKO_WINDOWS) || defined(HAXE3DS) || defined(NEKO_MAC)) && !defined(MSG_NOSIGNAL)
...
namespace
{
   ...
   SOCKET val_sock(Dynamic inValue)
   {
      ...
      return old_socket; // return self socket
   }
...
Dynamic _hx_std_socket_new(bool udp, bool ipv6)
{
   ...
   old_socket = s; // so that it wont throw stupid errors.
#ifdef NEKO_MAC
   int set = 1;
...
#if defined(NEKO_WINDOWS) || defined(NEKO_MAC) || defined(BLACKBERRY) || defined(EMSCRIPTEN) || defined(HAXE3DS)
...
String _hx_std_host_reverse(int host)
{
   ...
   hx::EnterGCFreeZone();
#if defined(NEKO_WINDOWS) || defined(NEKO_MAC) || defined(ANDROID) || defined(BLACKBERRY) || defined(EMSCRIPTEN) || defined(HAXE3DS)
...
void _hx_std_socket_set_timeout(Dynamic o, Dynamic t)
{
   ...
   hx::EnterGCFreeZone();
   if (setsockopt(sock, SOL_SOCKET, 0x1005, (char *)&time, sizeof(time)) != 0)
   {
      hx::ExitGCFreeZone();
      return;
   }
